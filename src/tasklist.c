  ...
  while (fgets(line, sizeof(line), file) && icon_count < MAX_ICONS) {
    ...
    if (sep) {
      *sep = '\0';
      ...
      snprintf(icon_table[icon_count].key, sizeof(icon_table[icon_count].key),
               "%.63s", line);
      snprintf(icon_table[icon_count].icon, sizeof(icon_table[icon_count].icon),
               "%.15s", icon_ptr);
...
const char *get_icon(const char *class_name) {
  // SÉCURITÉ : Si le pointeur est NULL, on s'arrête tout de suite
  if (class_name == NULL) {
    return ICON_DEFAULT;
  }
  ...
  if (strstr(class_name, "term") || strstr(class_name, "shell"))
    return ICON_TERM;
  if (strstr(class_name, "browser") || strstr(class_name, "web"))
    return ICON_WEB;
  if (strstr(class_name, "edit") || strstr(class_name, "write"))
    return ICON_EDIT;
  if (strstr(class_name, "file") || strstr(class_name, "fm"))
    return ICON_FM;
  if (strstr(class_name, "player") || strstr(class_name, "video"))
    return ICON_MEDIA;
  if (strstr(class_name, "config") || strstr(class_name, "settings"))
    return ICON_GEAR;
...
int main() {
  ...
  while (fgets(line, sizeof(line), fw)) {
    ...
    if (sscanf(line, "%31s %7s %255s %255s %511[^\n]", id, desk, class, host,
      ...
      for (int i = 0; class_low[i]; i++)
        class_low[i] = (unsigned char)tolower(class_low[i]);
      ...
      if (strcmp(clean_current_id, active_win_id) == 0) {
        printf("%%{F%s}%%{u%s}%%{+u}  %s  %%{-u}%%{F-} ", COLOR_ACTIVE,
               COLOR_ACTIVE, icon);
      } else {
        printf("%%{A1:wmctrl -i -a %s:}%%{F%s}  %s  %%{F-}%%{A} ", id,
               COLOR_INACTIVE, icon);
      }
    }
  }
