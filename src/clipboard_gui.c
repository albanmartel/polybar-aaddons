...
#define PROGRAMME_NAME "clipboard_gui"
...
// --- DÉTERMINATION DU CHEMIN DE L'HISTORIQUE ---
// Doit correspondre EXACTEMENT au chemin utilisé par votre démon C
static void get_history_path(char *path, size_t max_len) {
  // 1. Sécurité : Vérification du pointeur NULL
  if (path == NULL) {
    return;
  }

  // 2. Sécurité : Vérification des bornes (Taille min pour notre fichier et max
  // pour Linux) Notre chaîne finale fera au maximum ~50-60 caractères, donc un
  // max_len < 64 est trop petit.
  if (max_len < 64 || max_len > PATH_MAX) {
    return;
  }
  ...
  if (runtime_dir && strlen(runtime_dir) > 0) {
    // Option principale : RAM propre à la session utilisateur
    snprintf(path, max_len, "%s/polybar_clipboard.hist", runtime_dir);
  } else {
    // Fallback sécurité : Dossier temporaire global (s'efface aussi au reboot)
    snprintf(path, max_len, "/tmp/polybar_clipboard.hist");
  }
}

// --- SYNCHRONISATION : RÉÉCRIRE LE FICHIER DEPUIS LA LISTBOX ---
// Cette fonction est appelée dès que vous modifiez, supprimez, ou videz
// l'interface
static void save_listbox_to_file(GtkWidget *listbox) {
  if (!listbox || !GTK_IS_LIST_BOX(listbox))
    return;

  char hist_path[512];
  get_history_path(hist_path, sizeof(hist_path));
  ...
  for (GList *iter = children; iter != NULL; iter = g_list_next(iter)) {
    ...
    ClipboardRowData *row_data = g_object_get_data(G_OBJECT(row), "row-data");

    if (row_data && row_data->full_text) {
      fprintf(f, "%s\n", row_data->full_text);
    }
  }
  if (children)
...
static void apply_style(void) {
  ...
  gtk_css_provider_load_from_data(
      ...
      "#eceff4; border-radius: 6px; border: 1px solid #4c566a; padding: 6px "
      "10px; margin: 2px; }"
...
// --- ACTION 2 : SUPPRIMER UNE LIGNE (ADAPTÉE) ---
static void on_delete_clicked(GtkButton *btn, gpointer data) {
  if (!GTK_IS_BUTTON(btn) || !data)
    return;

  const ClipboardRowData *row_data = (const ClipboardRowData *)data;
  GtkWidget *listbox = row_data->listbox;

  if (row_data->row_widget && GTK_IS_WIDGET(row_data->row_widget)) {
    // Le widget parent de row_box est la GtkListBoxRow
    GtkWidget *row_container = gtk_widget_get_parent(row_data->row_widget);
    if (row_container) {
      gtk_widget_destroy(row_container);
      // On sauvegarde le fichier après destruction du widget
      save_listbox_to_file(listbox);
    }
  }
}

// --- ACTION 3 : MODIFIER UNE LIGNE (ADAPTÉE) ---
static void on_edit_clicked(GtkButton *btn, gpointer data) {
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    ...
    if (new_text && strlen(new_text) > 0) {
      ...
      if (children) {
        if (GTK_IS_LABEL(children->data)) {
          ...
          if (strlen(new_text) > 55) {
            display_text[55] = '\0';
            g_strlcat(display_text, "...", sizeof(display_text));
          }
      ...
      // Appliquer les changements au fichier d'historique
      save_listbox_to_file(row_data->listbox);

      // Copier la version éditée
      GtkClipboard *clipboard = gtk_clipboard_get(GDK_SELECTION_CLIPBOARD);
...
// --- ACTION 4 : SUPPRIMER TOUT L'HISTORIQUE (ADAPTÉE) ---
static void on_clear_all_clicked(GtkButton *btn, gpointer data) {
  ...
  // Écrase le fichier pour le rendre vide
  char hist_path[512];
  get_history_path(hist_path, sizeof(hist_path));
  FILE *f = fopen(hist_path, "w");
  if (f)
...
// --- CHARGEMENT DE L'HISTORIQUE DEPUIS VOTRE FICHIER ---
static void load_custom_history(GtkWidget *listbox) {
  ...
  if (!fp)
    return; // Si le fichier n'existe pas encore, on sort sans erreur

  char line[4096]; // Augmenté à 4096 pour correspondre à la taille du démon C
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (utf8_valid_text != NULL) {
      ...
      gtk_widget_set_tooltip_text(btn_copy, "Copier");
      ...
      gtk_widget_set_tooltip_text(btn_edit, "Modifier");
...
int creer_interface_presse_papiers(int argc, char *argv[]) {
  ...
  GtkWidget *btn_clear_all =
      gtk_button_new_with_label("🚨 Tout effacer l'historique");
  if (btn_clear_all && listbox) {
    gtk_style_context_add_class(gtk_widget_get_style_context(btn_clear_all),
                                "btn-danger");
    g_signal_connect(btn_clear_all, "clicked", G_CALLBACK(on_clear_all_clicked),
                     listbox);
    gtk_box_pack_end(GTK_BOX(vbox), btn_clear_all, FALSE, FALSE, 5);
  }

  if (listbox) {
    load_custom_history(listbox);
  }
...
int main(int argc, char *argv[]) {
  return creer_interface_presse_papiers(argc, argv);
}
