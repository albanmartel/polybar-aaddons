...
LogWindow* create_log_window(const char *title) {
    ...
    gtk_css_provider_load_from_data(provider, "textview text { background-color: #1a1a1a; color: #00d9ff; font-family: monospace; }", -1, NULL);
    gtk_style_context_add_provider(gtk_widget_get_style_context(lw->text_view), GTK_STYLE_PROVIDER(provider), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
...
void on_update_clicked(GtkButton *b, gpointer win) {
    ...
    run_command_in_log_window("pkexec pacman -Syu --noconfirm", "Mise à jour Système (Pacman)");
}

void on_export_clicked(GtkButton *b, gpointer win) {
    (void)b;
    GtkWidget *dialog = gtk_file_chooser_dialog_new("Exporter liste Pacman", GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_SAVE, "_Annuler", GTK_RESPONSE_CANCEL, "_Enregistrer", GTK_RESPONSE_ACCEPT, NULL);
...
void on_import_clicked(GtkButton *b, gpointer win) {
    (void)b;
    GtkWidget *dialog = gtk_file_chooser_dialog_new("Importer liste Pacman", GTK_WINDOW(win), GTK_FILE_CHOOSER_ACTION_OPEN, "_Annuler", GTK_RESPONSE_CANCEL, "_Ouvrir", GTK_RESPONSE_ACCEPT, NULL);

    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        char *fn = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
        char cmd[2048];
        // On lit le fichier et on passe tout à pacman via pkexec
        snprintf(cmd, sizeof(cmd), "pkexec pacman -S --needed --noconfirm $(cat '%s')", fn);
        gtk_widget_destroy(GTK_WIDGET(win));
...
int main(int argc, char *argv[]) {
    ...
    gtk_button_set_image(GTK_BUTTON(btn_upd), gtk_image_new_from_icon_name("system-software-update", GTK_ICON_SIZE_BUTTON));
    
    GtkWidget *btn_exp = gtk_button_new_with_mnemonic("_Exporter Paquets");
    gtk_button_set_image(GTK_BUTTON(btn_exp), gtk_image_new_from_icon_name("document-export", GTK_ICON_SIZE_BUTTON));
    
    GtkWidget *btn_imp = gtk_button_new_with_mnemonic("_Importer & Installer");
    gtk_button_set_image(GTK_BUTTON(btn_imp), gtk_image_new_from_icon_name("document-import", GTK_ICON_SIZE_BUTTON));
